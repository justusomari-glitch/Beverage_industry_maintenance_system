...
with st.sidebar:
    ...
    temperature= st.number_input("Temperature (C)", value=75.0, step=0.1)
    vibration= st.number_input("Vibration (mm/s)", value=2.5, step=0.01)
    pressure= st.number_input("Pressure (bar)", value=4.2, step=0.1)
    rotation_speed= st.number_input("Rotation Speed (RPM)", value=1450.0, step=1.0)
    torque= st.number_input("Torque (N-m)", value=75.0, step=0.1)
    voltage= st.number_input("Voltage (V)", value=220.0, step=0.1)
    current= st.number_input("Current (A)", value=12.5, step=0.1)
    power_consumption= st.number_input("Power Consumption (kW)", value=3.2, step=0.01)
    humidity= st.number_input("Humidity (%)", value=55.0, step=0.1)
    noise_level= st.number_input("Noise Level (dB)", value=68.0, step=0.1)
    flow_rate= st.number_input("Flow Rate (L/min)", value=12.0, step=0.1)
    product_temperature= st.number_input("Product Temperature (C)", value=6.0, step=0.1)
    oil_level= st.number_input("Oil Level (%)", value=80.0, step=0.1)
    bearing_temperature= st.number_input("Bearing Temperature (C)", value=55.0, step=0.1)

    st.markdown('<div class="selection-label">Operational Parameters</div>',unsafe_allow_html=True)
    cleaning_cycle_status= st.selectbox("Cleaning Cycle Status",[0,1], format_func=lambda x: "Active" if x else "Inactive")
    hours_since_maintenance= st.number_input("Hours Since Maintenance (C)", value=120, step=1,min_value=0)
