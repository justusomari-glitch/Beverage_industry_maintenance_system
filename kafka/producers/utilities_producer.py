...
UTILITIES_MACHINES=MACHINES_DF[
    MACHINES_DF['section'] == "Utilities"
].to_dict("records")
...
def simulate_sensors():
    return {
        ...
        "hours_since_maintenance": random.randint(0,200),
...
def run():
    ...
    print(f"Utilities Producer Started - {len(UTILITIES_MACHINES)} machines sending data to topic '{topic}'")

    while True:
        for machine in UTILITIES_MACHINES:
            payload = {
                #metadata
