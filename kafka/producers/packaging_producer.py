...
PACKAGING_MACHINES=MACHINES_DF[
    MACHINES_DF['section'] == "Packaging"
].to_dict("records")
...
def run():
    ...
    print(f"Packaging Producer Started - {len(PACKAGING_MACHINES)} machines sending data to topic '{topic}'")

    while True:
        for machine in PACKAGING_MACHINES:
            payload = {
                #metadata
